	...
	public ArrayList<Livraison> run() {
		...
		for (int i = 1; i < livraisons.size() + 1; i++) {
			for (int j = 1; j < livraisons.size() + 1; j++) {
				distances[i][j] = getDistance(livraisons.get(i - 1)
						.getDestinataire().getCoordGPS(), livraisons.get(j - 1)
						.getDestinataire().getCoordGPS());
			}
		}

		for (int i = 1; i < livraisons.size() + 1; i++) {
			distances[0][i] = distances[i][0] = getDistance(coordGPSRepository,
					livraisons.get(i - 1).getDestinataire().getCoordGPS());
		}
